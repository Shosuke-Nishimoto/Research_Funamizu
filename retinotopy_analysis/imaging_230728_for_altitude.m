...
load('230722_Blue_altitude_4.mat')
load('NI_22-Jul-2023_123_59.bin.mat');
% rest_duration = find(ch(7,:,:)>0.2, 1 ) - find(ch(1,:,:)>4, 1 );
rest_duration = find(ch(7,:,:)<0.2, 1 ) - find(ch(1,:,:)>4, 1 ); % 今回だけたまたま
...
av_image_data_final = damy./repeat_num;

%%
figure;
plot(1:900, av_image_data_final(:,:))
...
av_data = zeros(size(image_data,1),num_frames,size(altitude_num,3));
std_data = zeros(size(image_data,1),num_frames,size(altitude_num,3));

% 端には刺激が出ていない
for i = 4:15
    for j = 1:num_frames
        temp = image_data(:,image_stim_align(2,ind_altitude(i,:))+j-100); % 230724 ここ変えた
...
[X,cmap] = imread("C:\Users\funam\Desktop\analysis\imaging\230722_Blue_altitude_tiff\230722_Blue_azimuth_tiff_X1.tif");
X = imresize(X, 1/4);
X = X(45:95,30:90);
X = ind2rgb(X,cmap);
% imshow(X);
...
L = size(av_image_data_final,2); % Length of signal
t = (0:L-1)*T; % Time vector
% フーリエ変換を実行
fourier = fft(av_image_data_final');

% 第1高調波成分を抽出
first_harmonic_idx = 2; % 第1高調波成分はインデックス2に対応する
first_harmonic = fourier(first_harmonic_idx,:);
% 第1高調波成分の位相を抽出
phase_angle = angle(first_harmonic);
phase_angle = reshape(phase_angle,size(data,1),size(data,2));

figure;
imshow(X);
hold on;
contour(phase_angle,'LevelStep', 0.1)
...
% % 端には刺激が出ていない
% for i = 4:15
%     array = cell2mat(responce_pixel_num_2(19-i));
%     if isempty(array)==0
%         temp = zeros(size(av_data_devide_2,3),1);
%         temp(array(1,:)) = 1;
%         temp = reshape(temp,size(data,1),size(data,2))+res_pixels_2(:,:,19-i);
%         res_R(temp==2) = colour(19-i,1);
%         res_G(temp==2) = colour(19-i,2);
%         res_B(temp==2) = colour(19-i,3);
%     end
% end
% figure;
% imshow(cat(3,res_R,res_G,res_B))
...
for i = 4:15
    ...
    if isempty(array)==0
        ...
        ttemp(temp==2) = 1;
        [centroids, shrunken_matrix] = shrink_regions_around_centroids(ttemp, 100);
        res_R(shrunken_matrix==1) = colour(19-i,1);
        res_G(shrunken_matrix==1) = colour(19-i,2);
        res_B(shrunken_matrix==1) = colour(19-i,3);
    end
end
figure;
imshow(cat(3,res_R,res_G,res_B))
