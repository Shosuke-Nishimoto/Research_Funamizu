...
azimuth_num = zeros(size(y,1), size(y,2), 36);
azimuth_num_RGB = zeros(size(y,1), size(y,2), 3, 36);
colour = zeros(36,3);
for i = 1:36
    if i <= 9
...
for i = 1:size(colour,1)
    ind = phi >= (-90+5*(i-1))/180*pi & phi < (-90+5*i)/180*pi;
...
load('230722_Blue_azimuth_2_4.mat')
load('NI_22-Jul-2023_201_03.bin.mat');
...
imshow(uint8(raw_data(30:98,50:99,5)))
%% df/fを計算
data = (raw_data-f)./f;
data = data(30:98,50:99,:);
...
[X,cmap] = imread("C:\Users\funam\Desktop\analysis\imaging\230722_Blue_azimuth_2_tiff\230722_Blue_altitude_2_tiff_X1.tif");
X = imresize(X, 1/4);
X = X(30:98,50:99);
X = ind2rgb(X,cmap);
% imshow(X);
