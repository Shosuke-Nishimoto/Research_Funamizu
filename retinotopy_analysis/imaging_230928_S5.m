...
for i = 1:iteNum
    ...
    raw_data(:,:,i) = temp(121:185,111:215);
...
load("D:\Nishimoto_data\ISOI\230926_S5_f1\230926_S5_f1.mat")
load("D:\Nishimoto_data\ISOI\230926_S5_f1\NI_27-Sep-2023_1722_14.bin.mat");
...
imshow(X);
hold on;
scatter(grid_x(:), grid_y(:), 3, phase_angle_altitude_f(:), 'filled');
colorbar
colormap('jet')
% clim([-pi -1])
hold off;
...
imshow(X);
hold on;
scatter(grid_x(:), grid_y(:), 3, phase_angle_azimuth_f(:), 'filled');
colorbar
colormap('jet')
% clim([pi/2 pi])
hold off;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clearvars -except filename pathname X phase_angle_altitude_f phase_angle_azimuth_f

load("D:\Nishimoto_data\ISOI\230926_S5_b1\230926_S5_b1.mat")
load("D:\Nishimoto_data\ISOI\230926_S5_b1\NI_27-Sep-2023_1739_10.bin.mat");
...
repeat_num_azimuth = size(image_data_azimuth,2)/max_stim_num;
for i = 1:repeat_num_azimuth
    f = mean(image_data_azimuth(:,max_stim_num*(i-1)+151:max_stim_num*(i-1)+211),2); % 空白の5sの後
    del_f = (image_data_azimuth(:,max_stim_num*(i-1)+1:max_stim_num*i)-f)./f;
    damy = damy + del_f;
end
av_image_data_azimuth = damy./repeat_num_azimuth;
...
repeat_num_altitude = size(image_data_altitude,2)/max_stim_num;
for i = 1:repeat_num_altitude
    f = mean(image_data_altitude(:,max_stim_num*(i-1)+151:max_stim_num*(i-1)+211),2); % 空白の5sの後
    del_f = (image_data_altitude(:,max_stim_num*(i-1)+1:max_stim_num*i)-f)./f;
    damy = damy + del_f;
end
av_image_data_altitude = damy./repeat_num_altitude;
...
imshow(X);
hold on;
scatter(grid_x(:), grid_y(:), 3, phase_angle_altitude_b(:), 'filled');
colorbar
colormap('jet')
% clim([-pi -1])
hold off;
...
imshow(X);
hold on;
scatter(grid_x(:), grid_y(:), 3, phase_angle_azimuth_b(:), 'filled');
colorbar
colormap('jet')
% clim([pi/2 pi])
hold off;
...
imshow(X);
hold on;
scatter(grid_x(:), grid_y(:), 3, alt_angle(:), 'filled');
colorbar
colormap('jet')
clim([-40 60])
hold off;
...
imshow(X);
hold on;
scatter(grid_x(:), grid_y(:), 3, azi_angle(:), 'filled');
...
%%
[Gmag_altitude,Gdir_altitude] = imgradient(alt_angle);
[Gmag_azimuth,Gdir_azimuth] = imgradient(azi_angle);

sign_map = sin((Gdir_altitude-Gdir_azimuth)*pi/180);
SD = 0.2;
sign_map(sign_map>1.5*SD) =1; sign_map(sign_map<-1.5*SD) = -1; sign_map(sign_map<=1.5*SD & sign_map>=-1.5*SD) =0;
figure;
imshow(X);
hold on;
