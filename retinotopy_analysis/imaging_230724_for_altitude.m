...
altitude_num = zeros(size(y,1), size(y,2), 36);
altitude_num_RGB = zeros(size(y,1), size(y,2), 3, 36);
colour = zeros(36,3);
for i = 1:36
    if i <= 9
        colour(i,:) = [1 1/9*(i-1) 0];
    elseif i <= 18
        colour(i,:) = [1-1/9*(i-10) 1 0];
    elseif i <= 27
        colour(i,:) = [0 1 1/9*(i-19)];
    else
        colour(i,:) = [0 1-1/9*(i-28) 1];
    end
end

for i = 1:size(colour,1)
    ind = theta >= (90-5*i)/180*pi & theta < (90-5*(i-1))/180*pi;
