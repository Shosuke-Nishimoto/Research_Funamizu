...
imshow(uint8(raw_data(:,:,5)))
figure;
imshow(uint8(raw_data(35:85,30:80,5)))

[X,cmap] = imread("C:\Users\funam\Desktop\analysis\imaging\230711_Blue_3_tiff_X1.tif");
X = imresize(X, 1/4);
X = X(35:85,30:80);
X = ind2rgb(X,cmap);
% imshow(X);
...
r = rem(length(image_stim_align), 2*max_stim_num);
q = (length(image_stim_align)-r)/(2*max_stim_num);

is_align_1 = zeros(2,q*max_stim_num);
is_align_2 = zeros(2,q*max_stim_num);
for i = 1:q
    is_align_1(:,max_stim_num*(i-1)+1:max_stim_num*i) = image_stim_align(:,2*max_stim_num*(i-1)+max_stim_num+1:2*max_stim_num*i);
    is_align_2(:,max_stim_num*(i-1)+1:max_stim_num*i) = image_stim_align(:,2*max_stim_num*(i-1)+1:2*max_stim_num*(i-1)+max_stim_num);
end
...
av_image_data_1 = image_data(:, is_align_1(2,:));
av_image_data_1 = av_image_data_1(:,1:size(av_image_data_1,2)-rem(size(av_image_data_1,2), max_stim_num));

damy = zeros(size(av_image_data_1,1), max_stim_num);
repeat_num = size(av_image_data_1,2)/max_stim_num;
for i = 1:repeat_num
    damy = damy + av_image_data_1(:,max_stim_num*(i-1)+1:max_stim_num*i);
end
av_image_data_final_1 = damy./repeat_num;

av_image_data_2 = image_data(:, is_align_2(2,:));
av_image_data_2 = av_image_data_2(:,1:size(av_image_data_2,2)-rem(size(av_image_data_2,2), max_stim_num));

damy = zeros(size(av_image_data_2,1), max_stim_num);
repeat_num = size(av_image_data_2,2)/max_stim_num;
for i = 1:repeat_num
    damy = damy + av_image_data_2(:,max_stim_num*(i-1)+1:max_stim_num*i);
end
av_image_data_final_2 = damy./repeat_num;

%%
figure;
plot(1:900, av_image_data_final_1(:,:))
figure;
plot(1:900, av_image_data_final_2(:,:))
...
L = size(av_image_data_final_1,2); % Length of signal
t = (0:L-1)*dt; % Time vector
ff = Fs*(0:(L/2))/L;
% フーリエ変換を実行
fourier_1 = fft(av_image_data_final_1');
fourier_2 = fft(av_image_data_final_2');

% 第1高調波成分を抽出
% first_harmonic_idx = 91; % 刺激は6Hzで出ているので6Hzのところを使う
first_harmonic_idx = 2; % 1sで14度を使う
first_harmonic_1 = fourier_1(first_harmonic_idx,:);
first_harmonic_2 = fourier_2(first_harmonic_idx,:);
% 第1高調波成分の位相を抽出
phase_angle_1 = angle(first_harmonic_1);
phase_angle_1 = reshape(phase_angle_1,size(data,1),size(data,2));
phase_angle_2 = angle(first_harmonic_2);
phase_angle_2 = reshape(phase_angle_2,size(data,1),size(data,2));
...
for i = 3:14
    for j = 1:num_frames
        temp = image_data(:,is_align_1(2,ind_azimuth(i,:))+j-num_frames/2); % 230724 ここ変えた
        all_data_1(:,j,:,i) = temp;
...
for i = 4:15
    for j = 1:num_frames
        temp = image_data(:,is_align_2(2,ind_altitude(i,:))+j-num_frames/2); % 230724 ここ変えた
        all_data_2(:,j,:,i) = temp;
...
%%
% 6Hzのみを使う, →1sで14度を使う
fourier_1_select = zeros(size(fourier_1,1), size(fourier_1,2));
fourier_1_select(86:96,:) = fourier_1(86:96,:);
fourier_2_select = zeros(size(fourier_2,1), size(fourier_2,2));
fourier_2_select(86:96,:) = fourier_2(86:96,:);
...
figure;
plot(1:900, av_image_data_final_1(1000,:))
figure;
