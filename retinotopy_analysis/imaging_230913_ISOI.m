[X,cmap] = imread("D:\Nishimoto_data\ISOI\230912_ISOI_1\230913_ISOI_1_tiff\230913_ISOI_1_X1.tif");
X = imresize(X, 1/8);
X = ind2rgb(X,cmap);
figure;
imshow(X)
...
imds = imageDatastore("D:\Nishimoto_data\ISOI\230912_ISOI_1\230913_ISOI_1_tiff");
im_list = imds.Files;
...
% load('230912_ISOI_1.mat')
% load('NI_12-Sep-2023_2045_18.bin.mat');
...
% [X,cmap] = imread("D:\Nishimoto_data\ISOI\230912_ISOI_1\230913_ISOI_1_tiff\230913_ISOI_1_X1.tif");
[X,cmap] = imread("D:\Nishimoto_data\ISOI\230912_ISOI_2\230912_ISOI_2_tiff\230912_ISOI_2_X1.tif");
X = imresize(X, 1/8);
X = ind2rgb(X,cmap);
...
% max-min >= 0.04のみをとってくる
selected_index = [];
for i = 1:size(av_image_data_azimuth,1)
    if max(av_image_data_azimuth(i,:)) - min(av_image_data_azimuth(i,:)) >= 0.01
...
alt_point_low = 720; %  30
alt_point_high = 360; % -30
azi_point_low = 840; % 50
azi_point_high = 240; % -50
...
figure;
hold on;
plot((1:max_stim_num)/60, av_image_data_azimuth(azimuth_center_low,:))
plot((1:max_stim_num)/60, av_image_data_azimuth(azimuth_center_high,:))
hold off;
figure;
hold on;
plot((1:max_stim_num)/60, av_image_data_altitude(altitude_center_low,:))
plot((1:max_stim_num)/60, av_image_data_altitude(altitude_center_high,:))
hold off;
