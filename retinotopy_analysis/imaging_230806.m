...
% imshow(X)
%%
% imds = imageDatastore("C:\Users\funam\Desktop\analysis\imaging\230806_Blue_tiff");
% im_list = imds.Files;
...
%% import data
load('230806_Blue.mat')
load('NI_06-Aug-2023_1611_30.bin.mat');
...
% カメラの撮影タイミング
num_images = [];
for i = 1:length(ch_new)-1
    if ch_new(1,i+1)>=4 && ch_new(1,i)<4 % ここはセットにより変わる
...
num_stimuli = [];
for i = 1:length(ch_new)-1
    if ch_new(7,i+1)>=0.2 && ch_new(7,i)<0.2 % ここはセットにより変わる
...
% df/fを計算
rest_duration = find(ch_new(7,:,:)>0.2, 1 ) - find(ch_new(1,:,:)>4, 1 );
...
is_align_azimuth = zeros(2,q*max_stim_num);
is_align_altitude = zeros(2,q*max_stim_num);
for i = 1:q
    is_align_azimuth(:,max_stim_num*(i-1)+1:max_stim_num*i) = image_stim_align(:,2*max_stim_num*(i-1)+max_stim_num+1:2*max_stim_num*i);
    is_align_altitude(:,max_stim_num*(i-1)+1:max_stim_num*i) = image_stim_align(:,2*max_stim_num*(i-1)+1:2*max_stim_num*(i-1)+max_stim_num);
end
...
image_data_azimuth = image_data_azimuth(:,1:size(image_data_azimuth,2)-rem(size(image_data_azimuth,2), max_stim_num));
...
image_data_altitude = image_data_altitude(:,1:size(image_data_altitude,2)-rem(size(image_data_altitude,2), max_stim_num));
...
%%
figure;
plot(1:900, av_image_data_azimuth(1:100,:))
figure;
plot(1:900, av_image_data_altitude(1:100,:))
...
% 第1高調波成分を抽出
first_harmonic_idx = 1; % 1sで14度を使う
...
imshow(X);
hold on;
contour(phase_angle_azimuth,'LevelStep', 0.1)
...
imshow(X);
hold on;
contour(phase_angle_altitude,'LevelStep', 0.1)
% contourf(phase_angle,'LevelStep', 0.1)
axis equal tight
colorbar
hold off;
...
% まずは0度の位置の刺激に反応しているピクセルを正しく抽出する df/fを正しく計算
zero_point = 386;

[azi_max, azimuth_center] = max(mean(av_image_data_azimuth(:,zero_point+121:zero_point+181),2)...
                                - mean(av_image_data_azimuth(:,zero_point-59:zero_point),2));
[al_max, altitude_center] = max(mean(av_image_data_altitude(:,zero_point+121:zero_point+181),2)...
...
for i = 1:size(av_image_data_azimuth,1)
    [cross_corr, lag] = xcorr(av_image_data_azimuth(azimuth_center,:), av_image_data_azimuth(i,:)); % 相互相関
...
for i = 1:size(av_image_data_altitude,1)
    [cross_corr, lag] = xcorr(av_image_data_altitude(altitude_center,:), av_image_data_altitude(i,:)); % 相互相関
    [max_corr, max_idx] = max(cross_corr);   % 最大相関とそのインデックス
    phase_shift_samples = lag(max_idx);      % 位相ズレ（サンプル数）
...
for i = 1:size(rest_image_data,1)
    [cross_corr, lag] = xcorr(rest_image_data(azimuth_center,:), rest_image_data(i,:)); % 相互相関
    [max_corr, max_idx] = max(cross_corr);   % 最大相関とそのインデックス
    phase_shift_samples = lag(max_idx);      % 位相ズレ（サンプル数）
